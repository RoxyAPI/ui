...
export interface RoxyAlmanacDayProps extends ElementAttrs {
...
export const RoxyAlmanacDay = React.forwardRef<HTMLElement | null, RoxyAlmanacDayProps>(
	function RoxyAlmanacDay({ data, className, style, mode, endpoint, method, publishableKey, baseUrl, submitUrl, submitContext, locationUrl, specUrl, lang, submitLabel, remember, attribution, hideReadings, hideSections, ...rest }, ref) {
		...
		React.useEffect(() => {
			...
			if (el && mode !== undefined) {
				(el as unknown as { mode: 'day' | 'month' | 'auspicious' }).mode = mode;
			}
		}, [mode, loaded]);
		...
		return React.createElement('roxy-almanac-day', {
			ref: internal,
			className,
			style,
			...rest,
