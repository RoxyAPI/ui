...
export const RoxyTarotSpread = React.forwardRef<HTMLElement | null, RoxyTarotSpreadProps>(
	function RoxyTarotSpread({ data, className, style, spread, endpoint, method, publishableKey, baseUrl, submitUrl, submitContext, locationUrl, specUrl, lang, submitLabel, remember, attribution, hideReadings, hideSections, ...rest }, ref) {
		...
		React.useEffect(() => {
			...
			if (el && spread !== undefined) {
				(el as unknown as { spread: 'three-card' | 'celtic-cross' | 'love' | 'career' | 'custom' | 'yes-no' | 'draw' }).spread = spread;
			}
		}, [spread, loaded]);
