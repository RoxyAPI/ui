...
export interface RoxyVastuMandalaProps extends ElementAttrs {
...
export const RoxyVastuMandala = React.forwardRef<HTMLElement | null, RoxyVastuMandalaProps>(
	function RoxyVastuMandala({ data, className, style, mode, endpoint, method, publishableKey, baseUrl, submitUrl, submitContext, locationUrl, specUrl, lang, submitLabel, remember, attribution, hideReadings, hideSections, ...rest }, ref) {
		...
		React.useEffect(() => {
			...
			if (el && mode !== undefined) {
				(el as unknown as { mode: 'mandala' | 'entrance' }).mode = mode;
			}
		}, [mode, loaded]);
		...
		return React.createElement('roxy-vastu-mandala', {
			ref: internal,
			className,
			style,
			...rest,
