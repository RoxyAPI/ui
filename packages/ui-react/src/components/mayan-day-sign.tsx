...
export interface RoxyMayanDaySignProps extends ElementAttrs {
...
export const RoxyMayanDaySign = React.forwardRef<HTMLElement | null, RoxyMayanDaySignProps>(
	function RoxyMayanDaySign({ data, className, style, mode, endpoint, method, publishableKey, baseUrl, submitUrl, submitContext, locationUrl, specUrl, lang, submitLabel, remember, attribution, hideReadings, hideSections, ...rest }, ref) {
		...
		React.useEffect(() => {
			...
			if (el && mode !== undefined) {
				(el as unknown as { mode: 'day' | 'chart' }).mode = mode;
			}
		}, [mode, loaded]);
		...
		return React.createElement('roxy-mayan-day-sign', {
			ref: internal,
			className,
			style,
			...rest,
