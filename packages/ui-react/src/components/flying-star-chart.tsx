...
export interface RoxyFlyingStarChartProps extends ElementAttrs {
...
export const RoxyFlyingStarChart = React.forwardRef<HTMLElement | null, RoxyFlyingStarChartProps>(
	function RoxyFlyingStarChart({ data, className, style, mode, endpoint, method, publishableKey, baseUrl, submitUrl, submitContext, locationUrl, specUrl, lang, submitLabel, remember, attribution, hideReadings, hideSections, ...rest }, ref) {
		...
		React.useEffect(() => {
			...
			if (el && mode !== undefined) {
				(el as unknown as { mode: 'natal' | 'annual' }).mode = mode;
			}
		}, [mode, loaded]);
		...
		return React.createElement('roxy-flying-star-chart', {
			ref: internal,
			className,
			style,
			...rest,
