...
export interface RoxyZodiacCardProps extends ElementAttrs {
...
export const RoxyZodiacCard = React.forwardRef<HTMLElement | null, RoxyZodiacCardProps>(
	function RoxyZodiacCard({ data, className, style, mode, endpoint, method, publishableKey, baseUrl, submitUrl, submitContext, locationUrl, specUrl, lang, submitLabel, remember, attribution, hideReadings, hideSections, ...rest }, ref) {
		...
		React.useEffect(() => {
			...
			if (el && mode !== undefined) {
				(el as unknown as { mode: 'sign' | 'animal' | 'daily' | 'compatibility' }).mode = mode;
			}
		}, [mode, loaded]);
		...
		React.useEffect(() => {
			const el = internal.current;
			if (el && submitContext !== undefined) {
				(el as unknown as { submitContext: Record<string, unknown> }).submitContext = submitContext;
			}
		}, [submitContext, loaded]);
		...
		return React.createElement('roxy-zodiac-card', {
			ref: internal,
			className,
			style,
			...rest,
