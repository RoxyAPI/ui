...
export const RoxyDashaTimeline = React.forwardRef<HTMLElement | null, RoxyDashaTimelineProps>(
	function RoxyDashaTimeline({ data, className, style, period, endpoint, method, publishableKey, baseUrl, submitUrl, submitContext, locationUrl, specUrl, lang, submitLabel, remember, attribution, hideReadings, hideSections, ...rest }, ref) {
		...
		React.useEffect(() => {
			...
			if (el && period !== undefined) {
				(el as unknown as { period: 'current' | 'major' | 'sub' | 'antara' | 'sookshma' | 'prana' }).period = period;
			}
		}, [period, loaded]);
