...
export interface RoxyKuaCardProps extends ElementAttrs {
...
export const RoxyKuaCard = React.forwardRef<HTMLElement | null, RoxyKuaCardProps>(
	function RoxyKuaCard({ data, className, style, mode, endpoint, method, publishableKey, baseUrl, submitUrl, submitContext, locationUrl, specUrl, lang, submitLabel, remember, attribution, hideReadings, hideSections, ...rest }, ref) {
		...
		React.useEffect(() => {
			...
			if (el && mode !== undefined) {
				(el as unknown as { mode: 'kua' | 'mansions' }).mode = mode;
			}
		}, [mode, loaded]);
		...
		return React.createElement('roxy-kua-card', {
			ref: internal,
			className,
			style,
			...rest,
