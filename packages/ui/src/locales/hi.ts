 ...
 * ## Monthly ephemeris
 *
 * **`ग्रह स्थिति` is what Hindi astrology sites print over exactly this table, and it beats the loanword.** Drik Panchang heads its Hindi planetary-position pages `निरयण ग्रह स्थिति` and `सायन ग्रह स्थिति`, and AstroSage heads its own `आज की ग्रह स्थिति` with the table columns `ग्रह | राशि | रेखांश | नक्षत्र | पद`. The transliterated `एफेमेरिस` exists but was rejected: the one page carrying it spells the same word two ways on itself, which is not a term a catalogue can stand on. `पंचांग` was rejected outright because it names the five-limb almanac, not a longitude table, and conflating the two is the exact error this component would be accused of.
 *
 * **`राशि परिवर्तन` for a sign change, not `गोचर`.** AstroSage uses the first for the event (`यह ग्रह लगभग ढाई वर्ष में राशि परिवर्तन करता है`, `गोचर ग्रह के राशि-परिवर्तन से होता है`) and uses `गोचर` for both the event AND the whole residency reading (`शनि के गोचर की अवधि`), so `गोचर` is ambiguous in the one slot where the ambiguity costs something. `वक्री` is the retrograde word a Hindi reader actually carries: Drik Panchang heads a page `2026 बुध वक्री` with the label `कुल वक्री दिन = 23`, and `अवधि` is the period noun AstroSage uses (`गोचर की अवधि`).
 *
 * **The ingress string is NOMINAL on purpose.** Hindi astrology writing treats a planet as an honorific and inflects the verb for it (`प्रवेश करेंगे`, `करने जा रहे हैं`), so any verb form forces an agreement decision the UI cannot make from a substituted name. Drik Panchang publishes the nominal form for every solar ingress in its calendar (`सूर्य का धनु से मकर राशि में प्रवेश`, `सूर्य का मकर से कुम्भ राशि में प्रवेश`), and it takes a bare sign name in front of `राशि`. AstroSage corroborates the phrase (`किसी ग्रह का एक राशि से दूसरी राशि में प्रवेश करना`).
 *
 * **`दिनांक`, never `तिथि`, and this is the dangerous one.** In a panchang or ephemeris context `तिथि` is the LUNAR day: Drik Panchang prints `तिथि: द्वादशी` on its own homepage and AstroSage lists it among the five limbs. A Gregorian date column headed `तिथि` would be read as Dwadashi. `दिनांक` is the header sanatanjyoti.com uses and the word Drik Panchang uses in its own date-format validation message.
 *
 * The empty state and the table caption are COMPOSED rather than lifted, following the patterns already in this file; no astrology page publishes either. Sanskrit register was kept out of both: `निरयण`, `सायन`, `रेखांश` and `स्पष्ट` are correct panchang vocabulary and wrong for a widget a lay reader opens.
 */
