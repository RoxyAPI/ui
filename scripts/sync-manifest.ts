#!/usr/bin/env bun
/**
 * Mirror the TypeScript manifest into apps/docs/manifest.js so the static docs site can read it without a build step. CI fails on drift.
 *
 * @remarks
 * Four browser globals are emitted from the same generated file: `window.ROXY_COMPONENTS` (the component manifest), `window.ROXY_ENDPOINT_BINDINGS` (the generated component-to-endpoint map), `window.ROXY_THEME_PRESETS` (the theme palette data), and `window.ROXY_LOCALES` (the language payloads that actually shipped). The demo's `components-manifest.js` reads the second to derive the Embed tab snippet for every endpoint-bound card, so a binding change flows to the demo with no per-card edit; `page.js` reads the third to drive the customizer preset picker, so a palette edit flows to the demo with no hand-copied hexes.
 */
import { readdir, writeFile } from 'node:fs/promises';
import { ENDPOINT_BINDINGS } from '../packages/ui/src/generated/endpoint-bindings.js';
import { ROXY_COMPONENTS } from '../packages/ui/src/manifest.js';
import {
	type PaletteName,
	type PaletteTokens,
	ROXY_PALETTES,
	SHARED_THEME,
} from '../packages/ui/src/styles/palettes.js';
import { widgetSnippets } from './widget-snippets.js';

/**
 * The canonical copy-paste snippets, from the SAME builder that writes them into `components-catalog.json`. The demo renders these rather than deriving its own, so the Embed tab and the roxyapi.com `/widgets` page can no longer disagree about the snippet form (they did, over whether the practitioner theme was opt-in).
 */
const CDN_BASE = 'https://cdn.jsdelivr.net/npm/@roxyapi/ui@latest/dist/cdn';
const THEME_URL =
	'https://cdn.jsdelivr.net/npm/@roxyapi/ui@latest/dist/styles/themes/practitioner.css';
const SNIPPETS: Record<string, ReturnType<typeof widgetSnippets>> = {};
for (const c of ROXY_COMPONENTS) {
	const head = ENDPOINT_BINDINGS[c.tag]?.[0];
	if (!head) continue;
	SNIPPETS[c.tag] = widgetSnippets({
		tag: c.tag,
		slug: c.slug,
		method: head.method,
		path: head.path,
		attrs: head.attrs,
		cdnBase: CDN_BASE,
		themeUrl: THEME_URL,
	});
}

const OUT_PATH = 'apps/docs/manifest.js';

/** The overrides the customizer applies for one mode: the palette tokens plus the mode-shared danger, in token order. `accent-ink` and `ring` derive from `accent`, so the demo never emits them. */
function presetMode(t: PaletteTokens, mode: 'light' | 'dark') {
	return {
		accent: t.accent,
		secondary: t.secondary,
		danger: SHARED_THEME.danger[mode],
		bg: t.bg,
		surface: t.surface,
		fg: t.fg,
		muted: t.muted,
		border: t.border,
	};
}

const THEME_PRESETS = Object.fromEntries(
	(Object.keys(ROXY_PALETTES) as PaletteName[]).map((name) => [
		name,
		{
			file: `${name}.css`,
			light: presetMode(ROXY_PALETTES[name].light, 'light'),
			dark: presetMode(ROXY_PALETTES[name].dark, 'dark'),
		},
	]),
);

/** The language payloads that exist, read from the source directory the build reads. Adding a language is one file, and the demo picker gains it with no edit here or in the page. */
async function shippedLocales(): Promise<string[]> {
	const entries = await readdir('packages/ui/src/locales', {
		withFileTypes: true,
	});
	return entries
		.filter((e) => e.isFile() && e.name.endsWith('.ts'))
		.map((e) => e.name.replace(/\.ts$/, ''))
		.sort();
}

async function main() {
	const locales = await shippedLocales();
	const body =
		'// Generated by scripts/sync-manifest.ts. Do not edit.\n' +
		`window.ROXY_COMPONENTS = ${JSON.stringify(ROXY_COMPONENTS, null, 2)};\n` +
		`window.ROXY_ENDPOINT_BINDINGS = ${JSON.stringify(ENDPOINT_BINDINGS, null, 2)};\n` +
		`window.ROXY_THEME_PRESETS = ${JSON.stringify(THEME_PRESETS, null, 2)};\n` +
		`window.ROXY_WIDGET_SNIPPETS = ${JSON.stringify(SNIPPETS, null, 2)};\n` +
		`window.ROXY_LOCALES = ${JSON.stringify(locales)};\n`;
	await writeFile(OUT_PATH, body);
	console.log(
		`Wrote ${OUT_PATH} (${ROXY_COMPONENTS.length} components, ${Object.keys(ENDPOINT_BINDINGS).length} bound tags, ${Object.keys(THEME_PRESETS).length} theme presets, ${locales.length} locales).`,
	);
}

main().catch((err) => {
	console.error(err);
	process.exit(1);
});
