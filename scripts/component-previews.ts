...
const OVERRIDES: Record<string, { scale?: number; chartWidth?: string }> = {
	...
	// than legible. The natal chart is the lead example on every surface, so its tile
	// should show the wheel rather than the header above it.
	...
	// numbers down with it. The cap covers the compass frame, so the plate inside
	// it is one line height narrower on each side.
	'vastu-mandala': { chartWidth: '240px' },
